...
def _canonical_candidate(tmp_path: Path) -> Path:
    ...
    source = ROOT / "packages/xaac-agent_1.0.0-7_amd64.deb"
...
def test_importer_rejects_noncanonical_artifact_without_modifying_project(tmp_path: Path) -> None:
    ...
    candidate = tmp_path / "candidate" / "xaac-agent_1.0.0-7_amd64.deb"
    candidate.parent.mkdir()
    shutil.copy2(ROOT / "packages/xaac-agent_1.0.0-7_amd64.deb", candidate)
    shutil.copy2(
        ROOT / "packages/xaac-agent_1.0.0-7_amd64.deb.provenance.json",
...
def test_importer_installs_canonical_deb_and_updates_profile_transactionally(tmp_path: Path) -> None:
    ...
    assert result.version == "1.0.0-7"
    assert result.artifact == "packages/xaac-agent_1.0.0-7_amd64.deb"
