...
def _canonical_candidate(tmp_path: Path) -> Path:
    ...
    source = ROOT / "packages/xaac-agent_1.1.0-1_amd64.deb"
...
def test_embedded_release_artifact_is_canonical() -> None:
    ...
    assert result.debian_version == "1.1.0-1"
...
def test_release_gate_is_machine_readable_and_accepts_canonical_release() -> None:
    ...
    assert payload["debian_version"] == "1.1.0-1"
...
def test_standalone_canonical_artifact_can_be_validated_before_import(tmp_path: Path) -> None:
    ...
    result = validate_canonical_release_artifact(
        candidate,
        expected_application_version="1.1.0",
...
def test_importer_rejects_noncanonical_artifact_without_modifying_project(tmp_path: Path) -> None:
    ...
    candidate = tmp_path / "candidate" / "xaac-agent_1.1.0-1_amd64.deb"
    candidate.parent.mkdir()
    shutil.copy2(ROOT / "packages/xaac-agent_1.1.0-1_amd64.deb", candidate)
    shutil.copy2(
        ROOT / "packages/xaac-agent_1.1.0-1_amd64.deb.provenance.json",
...
def test_importer_installs_canonical_deb_and_updates_profile_transactionally(tmp_path: Path) -> None:
    ...
    assert result.version == "1.1.0-1"
    assert result.artifact == "packages/xaac-agent_1.1.0-1_amd64.deb"
...
def test_production_iso_wrapper_requires_canonical_release_before_system_tools() -> None:
    ...
    final_gate = wrapper.index('"$PROJECT_ROOT/scripts/validate-block10-release.sh"')
