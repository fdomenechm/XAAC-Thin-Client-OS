...
def test_font_configuration_is_written_idempotently(tmp_path: Path, project_root: Path) -> None:
    ...
    before = (fontconfig.read_text(encoding="utf-8"), gtk_settings.read_text(encoding="utf-8"))
    configurator.execute(plan)
    after = (fontconfig.read_text(encoding="utf-8"), gtk_settings.read_text(encoding="utf-8"))
    assert after == before
