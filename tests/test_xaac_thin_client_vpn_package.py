...
def test_session_supervisor_uses_independent_session_entry(project_root: Path) -> None:
    assert "client_command: /usr/local/libexec/xaac-session-entry" in (project_root/"config/session-supervisor.yaml").read_text()
    entry_source = (project_root / "src/xaac_thin_client_os/session_supervisor.py").read_text()
    assert "xaac-vpn-session-gate" not in entry_source
    assert 'REMOTE=/usr/bin/xaac-thinclient' in entry_source
...
def test_vpn_profile_no_longer_declares_a_session_gate(project_root: Path) -> None:
    profile = yaml.safe_load((project_root / "config/xaac-thin-client-vpn-package.yaml").read_text())
    assert "session_gate" not in profile["runtime"]
