...
def _artifact(project_root: Path) -> Path:
    return project_root / "packages/xaac-agent_1.1.0-1_amd64.deb"
...
def test_load_agent_profile_declares_debian_owned_resources(project_root: Path) -> None:
    ...
    assert profile["package"]["application_version"] == "1.1.0"
    assert profile["package"]["version"] == "1.1.0-1"
    assert profile["ownership"]["configuration_root"] == "/etc/xaac-agent"
    assert profile["ownership"]["runtime_root"] == "/usr/lib/python3/dist-packages/xaac_agent"
    ...
    assert "/usr/bin/xaac-agent" in profile["installation"]["required_paths"]
...
def test_real_agent_artifact_matches_profile(project_root: Path, tmp_path: Path) -> None:
    ...
    assert plan.metadata.version == "1.1.0-1"
...
def test_verification_requires_secure_admin_enrollment_contract(project_root: Path, tmp_path: Path) -> None:
    ...
    assert "head -n1 /usr/sbin/xaac-agent-admin" in command
    assert "/opt/xaac-agent/runtime" not in command
...
def test_installer_delegates_installation_to_deb(project_root: Path, tmp_path: Path) -> None:
    ...
    assert "1.1.0-1" in manifest.read_text(encoding="utf-8")
