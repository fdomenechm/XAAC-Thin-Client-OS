...
if [ -z "$ARTIFACT" ]; then
    printf 'Ús: %s %s\n' "$0" '/ruta/al/xaac-thin-client-vpn_VERSION_all.deb' >&2
    exit 2
fi
...
try:
    result = import_component_release(Path(sys.argv[1]), Path(sys.argv[2]), component="vpn")
except ComponentReleaseImportError as exc:
    print(json.dumps({"schema": "xaac-component-release-import/v1", "imported": False, "component": "vpn", "error": str(exc)}, sort_keys=True))
    raise SystemExit(1)
