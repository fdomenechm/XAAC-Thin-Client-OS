...
if [ -z "$ARTIFACT" ]; then
    printf 'Ús: %s %s\n' "$0" '/ruta/al/xaac-thin-client-network_VERSION_all.deb' >&2
    exit 2
fi
...
try:
    result = import_component_release(Path(sys.argv[1]), Path(sys.argv[2]), component="network")
except ComponentReleaseImportError as exc:
    print(json.dumps({"schema": "xaac-component-release-import/v1", "imported": False, "component": "network", "error": str(exc)}, sort_keys=True))
    raise SystemExit(1)
