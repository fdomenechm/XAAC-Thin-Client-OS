...
class ProductionIsoBuilder:
    ...
    def phase_configure(self) -> None:
        ...
        self._atomic_write(
            ...
            "set -eu\n"
            "clear\n"
            "printf '%s\\n' 'XAAC Thin Client OS — Instal·lador (pas 2)'\n"
            "printf '%s\\n' '============================================'\n"
            "printf '\\n'\n"
            "printf '%s\\n' 'Detecció i selecció no destructiva del disc de destinació.'\n"
            "printf '%s\\n' 'Aquest pas NO particiona, formata ni modifica cap disc.'\n"
            "printf '\\n'\n"
            "disks_file=$(mktemp)\n"
            "trap 'rm -f \"$disks_file\"' EXIT HUP INT TERM\n"
            "lsblk -dnpo NAME,SIZE,MODEL,TYPE,RO,RM | awk '$4 == \"disk\" && $5 == 0 {print}' > \"$disks_file\"\n"
            "if [ ! -s \"$disks_file\" ]; then\n"
            "    printf '%s\\n' 'No s’ha detectat cap disc intern escrivible.'\n"
            ...
            "    name=$(printf '%s\\n' \"$line\" | awk '{print $1}')\n"
            "    size=$(printf '%s\\n' \"$line\" | awk '{print $2}')\n"
            "    model=$(printf '%s\\n' \"$line\" | awk '{$1=$2=$NF=$(NF-1)=\"\"; sub(/^ +/, \"\"); sub(/ +$/, \"\"); print}')\n"
            ...
            "printf '\\n'\n"
            "printf '%s\\n' 'Disc seleccionat correctament:'\n"
            "printf '  %s (%s)\\n' \"$selected_name\" \"$selected_size\"\n"
            "printf '\\n'\n"
            "printf '%s\\n' 'Aquest és encara un pas de prova no destructiu.'\n"
            "printf '%s\\n' 'No s’ha escrit cap dada al disc seleccionat.'\n"
            "printf '\\n'\n"
            "printf '%s' 'Premeu Retorn per reiniciar el sistema: '\n"
            "IFS= read -r _answer\n"
            "systemctl reboot\n",
