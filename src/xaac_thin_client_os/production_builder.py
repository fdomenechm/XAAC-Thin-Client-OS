...
class ProductionIsoBuilder:
    ...
    def phase_configure(self) -> None:
        ...
        self._atomic_write(
            ...
            "set -eu\n"
            "clear\n"
            "printf '%s\\n' 'XAAC Thin Client OS — Instal·lador (pas 3)'\n"
            "printf '%s\\n' '============================================'\n"
            "printf '\\n'\n"
            "printf '%s\\n' 'Selecció, validació de seguretat i confirmació del disc.'\n"
            ...
            "printf '\\n'\n"
            "printf '%s\\n' 'En la fase següent, totes les dades d’aquest disc s’eliminaran.'\n"
            ...
            "fi\n"
            "printf '\\n'\n"
            "printf '%s\\n' 'Disc validat i confirmació acceptada.'\n"
            "printf '%s\\n' 'Aquest pas continua sent no destructiu: no s’ha escrit cap dada.'\n"
            "printf '\\n'\n"
            "printf '%s' 'Premeu Retorn per reiniciar el sistema: '\n"
            "IFS= read -r _answer\n"
            "systemctl reboot\n",
        ...
        self._atomic_write(
            ...
            "[Unit]\n"
            "Description=XAAC Thin Client OS installer safety validation (step 3)\n"
