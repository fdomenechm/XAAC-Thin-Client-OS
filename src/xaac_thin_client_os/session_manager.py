...
def create_session_manager_plan(rootfs: Path, profile_path: Path) -> SessionManagerPlan:
    ...
    launcher = (
        ...
        "# Session infrastructure needs the standard root-owned system tools.\n"
        "# Only the Dock receives XAAC_KIOSK_PATH for component discovery.\n"
        "export XAAC_SYSTEM_PATH=/usr/sbin:/usr/bin:/sbin:/bin\n"
        "export XAAC_KIOSK_PATH=/usr/local/libexec/xaac:/usr/libexec/xaac\n"
        "export PATH=$XAAC_SYSTEM_PATH\n"
        "# Propagate the installer/admin keyboard choice using POSIX shell built-ins\n"
        "# only, so the launcher cannot fail before the compositor because of PATH.\n"
        "xaac_xkb_layout=\n"
        "xaac_xkb_variant=\n"
        "xaac_xkb_model=\n"
        "if [ -r /etc/default/keyboard ]; then\n"
        "    while IFS='=' read -r xaac_key xaac_value || [ -n \"$xaac_key$xaac_value\" ]; do\n"
        "        case $xaac_value in\n"
        "            \\\"*\\\") xaac_value=${xaac_value#\\\"}; xaac_value=${xaac_value%\\\"} ;;\n"
        "            \\\'*\\\') xaac_value=${xaac_value#\\\'}; xaac_value=${xaac_value%\\\'} ;;\n"
        "        esac\n"
        "        case $xaac_key in\n"
        "            XKBLAYOUT) xaac_xkb_layout=$xaac_value ;;\n"
        "            XKBVARIANT) xaac_xkb_variant=$xaac_value ;;\n"
        "            XKBMODEL) xaac_xkb_model=$xaac_value ;;\n"
        "        esac\n"
        "    done < /etc/default/keyboard\n"
        "fi\n"
        "case $xaac_xkb_layout in ''|*[!A-Za-z0-9_,+-]*) xaac_xkb_layout=us ;; esac\n"
        "case $xaac_xkb_variant in *[!A-Za-z0-9_,+-]*) xaac_xkb_variant= ;; esac\n"
        "case $xaac_xkb_model in ''|*[!A-Za-z0-9_-]*) xaac_xkb_model=pc105 ;; esac\n"
        "export XKB_DEFAULT_LAYOUT=$xaac_xkb_layout\n"
        "export XKB_DEFAULT_VARIANT=$xaac_xkb_variant\n"
        "export XKB_DEFAULT_MODEL=$xaac_xkb_model\n"
    ...
    environment = (
        ...
        "XCURSOR_SIZE=24\n"
        "XAAC_SYSTEM_PATH=/usr/sbin:/usr/bin:/sbin:/bin\n"
        "XAAC_KIOSK_PATH=/usr/local/libexec/xaac:/usr/libexec/xaac\n"
